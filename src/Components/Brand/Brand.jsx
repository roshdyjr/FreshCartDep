    ...
    return (
        <div className="max-w-2xl mx-auto">
            <div className="bg-background-secondary dark:bg-background-primary shadow-md rounded-lg max-w-sm flex flex-col justify-between h-full w-full">
                <div>
                    <img className="rounded-t-lg p-8 w-full object-cover hover:scale-105 transition-all duration-300" src={brand.image} alt={brand.name} />
                </div>
                <div className="px-5 pb-5 flex flex-col justify-between flex-grow">
                    <div>
                        <Link to={"/brandProducts/" + brand._id}>
                            <h3 className="text-secondary-theme dark:text-background-secondary font-semibold text-xl tracking-tight text-center">{brand.name}</h3>
                        </Link>
                    </div>
                    <Link to={"/brandProducts/" + brand._id}
